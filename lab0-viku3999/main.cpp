...
int main(int argc, char *argv[]){
    ...
    char* ip_file=0;
    char* op_file=0;  
    int i_flag=0, o_flag=0, alg_flag=0;
    ...
    */
    for(int i=0; i<argc; i++){
        ...
        if(argv_str.find("--alg") != string::npos){
            if(argv_str.find("=quick") != string::npos){
                if(alg_flag){
            ...
            else if(argv_str.find("=merge") != string::npos){
                if(alg_flag){
                    cout<<"Multiple sorting algorithms detected. Please choose only one\n";
                    return 0;
                }
    ...
    */
    ...
    if(alg_flag == 0){
        cout<<"Please give an sorting algorithm and try again\n";
        return 0;
    }
    ...
    if(alg_flag == 2){
        if(merge_sort(sort_arr, 0, gh-1) == 0){
